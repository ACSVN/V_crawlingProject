...
DELETE FROM TmpTable2 WHERE PostalCode = "-";
...
INSERT INTO TmpTable2 SELECT p.id, p.URL, "-", 0, p.Title, p.CompanyName, p.Workplace, p.HeadOffice, p.PostalCode, p.Address, p.Recruiter, p.Phone, p.Fax, p.Email, p.Industry, p.Remarks, p.PostPeriod, p.Rank, p.Website FROM TmpTable1 as p WHERE NOT EXISTS(SELECT * FROM TmpTable2 as r WHERE p.id = r.id) GROUP BY p.URL ORDER BY p.id;
